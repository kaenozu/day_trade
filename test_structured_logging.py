...
def test_context_logging():
    ...
    logger = get_context_logger(
        "test_context", component="trading_system", version="1.0.0"
    )
...
def test_performance_logging():
    ...
    log_performance_metric(
        "api_response_time", 250, "ms", api_endpoint="/api/stock_data", success=True
    )

    log_performance_metric(
        "memory_usage", 45.6, "percentage", component="data_processor"
    )
...
def test_system_logging():
    ...
    # データベース操作ログ
    log_database_operation(
        "INSERT", "trades", rows_affected=1, execution_time=45, query_size=256
    )

    # システムヘルス情報
    log_system_health(
        "database", "healthy", connection_count=5, response_time=12, cpu_usage=25.3
    )
...
def test_security_logging():
    ...
    # ユーザーアクション
    log_user_action(
        "login", "user123", ip_address="192.168.1.100", user_agent="DayTradeApp/1.0"
    )

    log_user_action(
        "trade_execution", "user123", symbol="7203", action="buy", amount=250000
    )
