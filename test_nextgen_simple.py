...
try:
    ...
    try:
        ...
        start_time = time.time()
        pred_result = engine.predict_next_gen(
            test_data, use_uncertainty=False, use_ensemble=False
        )
        ...
        if (
            "predictions" in pred_result
            and "hybrid_lstm_transformer" in pred_result["predictions"]
        ):
            predictions = pred_result["predictions"]["hybrid_lstm_transformer"][
                "predictions"
            ]
            print(f"   予測数: {len(predictions)}")
            print(
                f"   予測サンプル: {predictions[:3] if len(predictions) >= 3 else predictions}"
            )
    ...
    try:
        ...
        print(
            f"   ハイブリッドモデル初期化済み: {summary['system_status']['hybrid_model_initialized']}"
        )
