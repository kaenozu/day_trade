...
from src.day_trade.utils.formatters import (
    format_currency, format_percentage, create_status_indicator,
    create_metric_cards, create_comparison_table
)
...
def demo_basic_alerts():
    ...
    # 価格アラートの追加
    price_alerts = [
        create_price_alert("toyota_3000", "7203", Decimal('3000'), above=True, priority=AlertPriority.HIGH),
        create_price_alert("sony_7500", "6758", Decimal('7500'), above=False, priority=AlertPriority.MEDIUM),
...
def demo_real_time_monitoring():
    ...
    def create_monitoring_display():
        ...
        for symbol, data in current_prices.items():
            change_color = "green" if data['change_percent'] > 0 else "red" if data['change_percent'] < 0 else "white"
        ...
        for alert_record in triggered_alerts[-5:]:  # 最新5件
            ...
            history_table.add_row(
                ...
                alert_record['symbol'],
                alert_record['message'][:50] + "..." if len(alert_record['message']) > 50 else alert_record['message'],
    ...
    try:
        with Live(create_monitoring_display(), refresh_per_second=2, screen=False) as live:
            # 監視開始
...
def demo_alert_analysis():
    ...
    alert_types = [AlertType.PRICE_ABOVE, AlertType.PRICE_BELOW, AlertType.CHANGE_PERCENT_UP, AlertType.VOLUME_SPIKE]
    ...
    for i in range(20):
        ...
        sample_history.append(AlertTrigger(
    ...
    for trigger in recent_alerts:
        ...
        change_color = "green" if trigger.change_percent > 0 else "red" if trigger.change_percent < 0 else "white"
        ...
        timeline_table.add_row(
            ...
            f"[{change_color}]{format_percentage(trigger.change_percent)}[/{change_color}]" if trigger.change_percent is not None else "N/A"
        )
