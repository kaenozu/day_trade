...
def generate_comprehensive_test_data() -> Dict[str, pd.DataFrame]:
    ...
    for symbol in test_symbols:
        ...
        high_prices = np.maximum(open_prices, close_prices) * np.random.uniform(1.0, 1.03, 252)
        low_prices = np.minimum(open_prices, close_prices) * np.random.uniform(0.97, 1.0, 252)
...
async def test_advanced_technical_indicators(test_data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
    """高度テクニカル指標テスト"""
    ...
    try:
        ...
        for symbol, data in list(test_data.items())[:3]:  # 3銘柄でテスト
            try:
                ...
                prediction = await system.predict_with_ensemble(data, symbol, feature_set)
