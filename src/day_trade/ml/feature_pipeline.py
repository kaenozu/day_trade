...
from .feature_store import FeatureStore, FeatureStoreConfig, create_feature_store
...
class FeaturePipeline:
    ...
    def __init__(self, config: PipelineConfig = None):
        ...
        logger.info("特徴量パイプライン初期化完了", extra={
            "config": {
    ...
    def generate_features_for_symbol(
        ...
        if not force_regenerate:
            ...
            if cached_result:
                ...
                logger.info("キャッシュから特徴量取得", extra={
                    "symbol": symbol,
                    "cache_hit": True,
        ...
        self.pipeline_stats['avg_generation_time'] = (
            (self.pipeline_stats['avg_generation_time'] * (self.pipeline_stats['total_requests'] - 1) + generation_time)
    ...
    def batch_generate_features(
        ...
        for batch_idx, symbol_batch in enumerate(symbol_batches):
            ...
            logger.info(f"バッチ {batch_idx + 1}/{len(symbol_batches)} 処理開始", extra={
            ...
            logger.info(f"バッチ {batch_idx + 1} 完了", extra={
                "processed_symbols": len(batch_results),
    ...
    def optimize_feature_config(
        ...
        symbol: str,
        data: pd.DataFrame,
        base_config: FeatureConfig = None
    ) -> FeatureConfig:
        """特徴量設定の最適化"""
