...
class CompressionConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'CompressionConfig':
        ...
        if "quantization_type" in data:
            config.quantization_type = QuantizationType(data["quantization_type"])
        if "pruning_type" in data:
            config.pruning_type = PruningType(data["pruning_type"])
        if "target_hardware" in data:
            config.target_hardware = HardwareTarget(data["target_hardware"])
            
        # その他の設定項目を更新
        for key, value in data.items():
            if hasattr(config, key) and not key.endswith("_type") and key != "target_hardware":
...
class CompressionResult:
    ...
    def __str__(self) -> str:
        ...
        return (
            ...
            f"速度向上: {self.speedup_ratio:.1f}x, "
            f"精度低下: {self.accuracy_drop:.3f})"
        )
...
def check_dependencies() -> Dict[str, bool]:
    ...
    try:
        ...
        dependencies["onnx_quantization"] = True
    except ImportError:
        dependencies["onnx_quantization"] = False
        warnings.warn("ONNX Runtime quantization tools not available", stacklevel=2)

    # TensorFlow Lite
    try:
        import tensorflow as tf
        dependencies["tf_lite"] = True
    except ImportError:
        dependencies["tf_lite"] = False
    ...
    try:
        ...
        dependencies["pytorch_quantization"] = True
    except ImportError:
        dependencies["pytorch_quantization"] = False

    # ハードウェア情報
    try:
        import cpuinfo
        dependencies["cpu_info"] = True
    except ImportError:
        dependencies["cpu_info"] = False

    logger.info(f"依存ライブラリ状況: {dependencies}")
    return dependencies
...
def get_model_size_mb(model_path: str) -> float:
    ...
    try:
        ...
        logger.warning(f"モデルサイズ取得失敗 ({model_path}): {e}")
        return 0.0


def validate_compression_config(config: CompressionConfig) -> bool:
    """圧縮設定の妥当性検証"""
    try:
        # 比率の範囲チェック
        if not (0.0 <= config.quantization_ratio <= 1.0):
            logger.error(f"量子化比率が範囲外: {config.quantization_ratio}")
            return False
            
        if not (0.0 <= config.pruning_ratio <= 1.0):
            logger.error(f"プルーニング比率が範囲外: {config.pruning_ratio}")
            return False
            
        # 目標値の妥当性
        if config.target_speedup_ratio <= 1.0:
            logger.warning(f"目標速度向上が1.0以下: {config.target_speedup_ratio}")
            
        if config.max_accuracy_drop < 0.0:
            logger.error(f"許容精度低下が負値: {config.max_accuracy_drop}")
            return False
