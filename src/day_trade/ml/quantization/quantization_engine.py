...
ONNX Runtime を使用したモデル量子化機能
- 動的量子化（Dynamic INT8 Quantization）
- 静的量子化（Static INT8 Quantization with Calibration）
- 混合精度量子化（FP16 Quantization）
- Issue #724対応: 強化FP16量子化版
"""
...
from .data_structures import CompressionConfig
...
try:
    ...
    warnings.warn(
        "ONNX Runtime quantization tools not available", stacklevel=2
    )
...
class ONNXQuantizationEngine:
    ...
    def apply_dynamic_quantization(
        ...
        Args:
            model_path: 入力ONNXモデルパス
            output_path: 出力量子化モデルパス
    ...
    def apply_mixed_precision_quantization(
        ...
        try:
            ...
            if success:
                ...
                fallback_success = self._fallback_graph_optimization(
    ...
    def _apply_fp16_quantization(
        ...
        try:
            ...
            try:
                return self._onnx_runtime_fp16_optimization(
                    model_path, output_path
                )
    ...
    def _update_graph_value_info_to_fp16(self, graph):
        ...
        try:
            ...
            for value_info in graph.input + graph.output + graph.value_info:
                if (
                    hasattr(value_info.type.tensor_type, "elem_type")
                    and value_info.type.tensor_type.elem_type
    ...
    def _onnx_runtime_fp16_optimization(
        self, model_path: str, output_path: str
    ) -> bool:
        ...
        try:
            ...
            logger.info("ONNX Runtime FP16最適化完了")
            return True
    ...
    def _verify_fp16_quantization(self, model_path: str) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"FP16量子化率: {fp16_ratio:.1%} "
                f"({fp16_tensors}/{total_tensors})"
            )
    ...
    def _fallback_graph_optimization(
        self, model_path: str, output_path: str
    ) -> bool:
        """フォールバック: 従来のグラフ最適化"""
        try:
        ...
        try:
            ...
            if successful_count == 1:
                ...
                stats["average_compression_ratio"] = (
                    (current_avg * (successful_count - 1) + ratio)
                    / successful_count
                )
        except:
            pass
