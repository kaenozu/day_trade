...
def generate_dummy_data(num_samples: int = 100, drift_point: int = 70) -> pd.DataFrame:
    ...
    actuals[drift_point:] = actuals[drift_point:] + np.random.rand(num_samples - drift_point) * 20 - 10 # ノイズを追加
    actuals[drift_point:] = actuals[drift_point:] + (predictions[drift_point:] - actuals[drift_point:]) * 0.5 # 予測との乖離を大きくする
...
def run_concept_drift_detection(data_path: Optional[str] = None):
    ...
    if data_path:
        try:
            ...
            logger.error(f"指定されたデータファイルが見つかりません: {data_path}。ダミーデータを生成します。")
            df = generate_dummy_data()
        except Exception as e:
            logger.error(f"データファイルのロード中にエラーが発生しました: {e}。ダミーデータを生成します。")
            df = generate_dummy_data()
    else:
        df = generate_dummy_data()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, help="Path to the CSV file containing predictions and actuals.", default=None)
