...
class MetricsCollector:
    ...
    def get_aggregated_metrics(self, minutes: int = 5) -> Dict:
        ...
        return {
            ...
            'memory': {
                'avg_usage_percent': round(sum(memory_usage) / len(memory_usage), 2) if memory_usage else 0,
                ...
                'avg_usage_gb': round((sum(m['memory']['used_bytes'] for m in history) / len(history)) / (1024**3), 2) if history else 0
            },

            'disk': {
                'avg_usage_percent': round(sum(disk_usage) / len(disk_usage), 2) if disk_usage else 0,
    ...
    def generate_health_report(self) -> Dict:
        ...
        if aggregated.get('cpu', {}).get('avg_usage_percent', 0) > 80:
            recommendations.append('CPU負荷が高くなっています。不要なプロセスの停止を検討してください')

        if aggregated.get('memory', {}).get('avg_usage_percent', 0) > 85:
            recommendations.append('メモリ使用量が多くなっています。メモリ使用量の最適化を検討してください')

        return {
            'report_timestamp': datetime.now().isoformat(),
            'overall_health': aggregated.get('performance_indicators', {}).get('system_health_score', 100),
