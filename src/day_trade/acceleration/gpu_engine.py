...
class GPUAccelerationEngine:
    ...
    def _cuda_technical_indicators(
        ...
        try:
            ...
            with cp.cuda.Device(device_id):
                # データを GPU メモリに転送
                close_prices = cp.asarray(data['Close'].values if 'Close' in data.columns 
                                         else data[data.columns[-1]].values)
                high_prices = cp.asarray(data['High'].values if 'High' in data.columns 
                                        else close_prices)
                low_prices = cp.asarray(data['Low'].values if 'Low' in data.columns 
                                       else close_prices)
                volume = cp.asarray(data['Volume'].values if 'Volume' in data.columns 
                ...
                for indicator in indicators:
                    if indicator == 'sma':
                        ...
                        results[indicator] = self._cuda_stochastic(high_prices, low_prices, close_prices, period)
    ...
    def _cpu_technical_indicators(
        ...
        close_prices = data['Close'].values if 'Close' in data.columns else data[data.columns[-1]].values
        results = {}
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_computation_time': total_time / max(self.performance_stats['total_computations'], 1),
    ...
    def benchmark_performance(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        speedup_ratio = cpu_time / gpu_result.execution_time if gpu_result.execution_time > 0 else 1.0
