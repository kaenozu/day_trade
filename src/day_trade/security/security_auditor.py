...
try:
    ...
    from .penetration_tester import PenetrationTester, PenTestConfig, SecurityVulnerability
except ImportError:
    import logging
    ...
    async def _run_bandit_analysis(self) -> List[AuditFinding]:
        ...
        try:
            cmd = [
                ...
                "-f", "json",
                "--skip", "B101,B601"  # assert_used, shell=True の一部除外
            ]
                ...
                try:
                    ...
                    for vuln in safety_data.get("vulnerabilities", []):
                        ...
                        finding = AuditFinding(
                            ...
                            description=vuln.get("advisory", "Vulnerable package dependency detected"),
            ...
            for container in containers:
                ...
                if config.get("User", "") in ["", "0", "root"]:
                    findings.append(AuditFinding(
                        ...
                        compliance_frameworks=[ComplianceFramework.CIS_CONTROLS],
                        result=AuditResult.FAIL,
                        evidence=[f"Container: {container.name}", f"User: {config.get('User', 'root')}"],
                        remediation_steps=[
...
class ComplianceAssessor:
    ...
    def _get_met_requirements(
        ...
        finding_categories = set(f.category for f in findings if framework in f.compliance_frameworks)
    ...
    def _get_framework_recommendations(
        ...
        if critical_high:
            recommendations.append(f"Address {len(critical_high)} critical/high severity findings immediately")
...
class SecurityAuditor:
    ...
    async def run_comprehensive_audit(self) -> SecurityReport:
        ...
        if AuditScope.APPLICATION in self.config.audit_scopes and self.config.enable_penetration_testing:
            pen_test_tasks = []
    ...
    def _perform_risk_assessment(self, findings: List[AuditFinding]) -> Dict[str, Any]:
        ...
        total_weighted_score = sum(
            severity_weights.get(f.severity, 1) * f.risk_rating
            for f in findings
        )

        max_possible_score = len(findings) * 10 * 10  # max severity * max risk rating
        risk_score = (total_weighted_score / max_possible_score) * 100 if max_possible_score > 0 else 0
