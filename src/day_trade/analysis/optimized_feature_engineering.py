...
class OptimizedAdvancedFeatureEngineer:
    ...
    def __init__(self, config: OptimizationConfig = None):
        ...
        self.feature_cache = {}
        self.executor = (
            ProcessPoolExecutor(max_workers=self.config.max_workers)
            if self.config.enable_parallel
            else None
        )
    ...
    def generate_composite_features(
        ...
        if self.config.enable_parallel and len(data) > self.config.chunk_size:
            feature_data = self._generate_features_parallel(
                feature_data, indicators, enable_advanced
            )
        else:
            feature_data = self._generate_features_sequential(
                feature_data, indicators, enable_advanced
            )
    ...
    def _generate_features_legacy_parallel(
        ...
        # データをチャンクに分割
        chunks = [
            data.iloc[i : i + self.config.chunk_size]
    ...
    def _add_price_features_optimized(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.config.enable_numba:
            ...
            for window in [10, 20, 50]:
                ...
                data[f"rel_position_{window}d"] = (data["Close"] - rolling_min) / (
    ...
    def _add_technical_features_optimized(
    ...
    def generate_market_features(
        self, feature_data: pd.DataFrame, market_data: Dict[str, pd.DataFrame]
    ) -> pd.DataFrame:
        """市場特徴量生成（軽量版）"""
        ...
        for market_name, market_df in market_data.items():
            if "Close" in market_df.columns:
                ...
                # ローリング相関（軽量版）
                correlation = stock_returns.rolling(20).corr(
                    market_returns.reindex(stock_returns.index, fill_value=0)
                )
...
def optimize_dataframe_memory(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in optimized_df.columns:
        ...
        if col_type == "float64":
            ...
            col_type == "int64"
            and optimized_df[col].min() >= np.iinfo(np.int32).min
