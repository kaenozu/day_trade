...
class TimeframeConfig:
    ...
    def __init__(self):
        ...
        self.analysis_indicators = ["sma", "ema", "bollinger_bands", "rsi", "macd"]
...
class MultiTimeframeAnalysisBase(OptimizationStrategy):
    ...
    def _analyze_timeframe(
        self, data: pd.DataFrame, tf_name: str, tf_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """単一タイムフレームの分析"""
        tf_result = {
            "timeframe": tf_name,
        ...
        # トレンド判定
        tf_result["trend"], tf_result["trend_strength"] = self._determine_trend(
            tf_result["indicators"], data
        )
...
class OptimizedMultiTimeframeAnalysis(MultiTimeframeAnalysisBase):
    ...
    def _analyze_multi_timeframe_parallel(
        self, data: pd.DataFrame, symbols: Optional[List[str]] = None, **kwargs
    ) -> MultiTimeframeResult:
        """並列マルチタイムフレーム分析"""
        result = MultiTimeframeResult()
        ...
        with ThreadPoolExecutor(
            max_workers=min(4, len(self.timeframe_config.timeframes))
        ) as executor:
            # 並列タスク投入
            future_to_timeframe = {}
            ...
            for tf_name, tf_config in self.timeframe_config.timeframes.items():
                future = executor.submit(
                    self._analyze_single_timeframe_cached, data, tf_name, tf_config
                )
    ...
    def _analyze_single_timeframe_cached(
        self, data: pd.DataFrame, tf_name: str, tf_config: Dict[str, Any]
    ...
    async def analyze_multi_timeframe_async(
        self, data: pd.DataFrame, symbols: Optional[List[str]] = None, **kwargs
    ) -> MultiTimeframeResult:
        """非同期マルチタイムフレーム分析"""
        result = MultiTimeframeResult()
    ...
    async def _analyze_timeframe_async(
        self, data: pd.DataFrame, tf_name: str, tf_config: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        """非同期単一タイムフレーム分析"""
