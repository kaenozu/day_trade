...
class ComprehensiveEnsembleTester:
    ...
    def run_comprehensive_ensemble_validation(
        ...
        try:
            ...
            # 推奨事項生成
            recommendations = self._generate_recommendations(test_results, voting_validation, weight_validation)
    ...
    def _generate_functional_test_cases(self) -> List[EnsembleTestCase]:
        ...
        test_cases.append(EnsembleTestCase(
        ...
        for config in self.ensemble_configs:
            for test_case in self.test_cases:
                try:
                    ...
                    logger.error(f"逐次テスト実行エラー: {e}", section="sequential_test_execution")

        return test_results

    # @profile_performance
    def _execute_single_test(self, test_case: EnsembleTestCase, config: Dict[str, Any]) -> EnsembleTestResult:
        """単一テスト実行"""
        ...
        try:
            ...
            if signal:
                confidence_in_range = (
                    test_case.expected_confidence_range[0] <= signal.confidence <= test_case.expected_confidence_range[1]
                )
    ...
    def _calculate_category_statistics(self, test_results: List[EnsembleTestResult]) -> Dict[str, Dict[str, int]]:
        """カテゴリ別統計計算"""
    ...
    def _calculate_performance_statistics(self, test_results: List[EnsembleTestResult]) -> Dict[str, float]:
        """パフォーマンス統計計算"""

        if not test_results:
            return {'avg_execution_time': 0, 'max_execution_time': 0, 'avg_memory_usage': 0, 'max_memory_usage': 0}
    ...
    def _generate_recommendations(
        ...
        if success_rate > 90:
            recommendations.append("アンサンブル戦略は高い信頼性を示しており、本番環境での使用を推奨")
        elif success_rate > 75:
            recommendations.append("アンサンブル戦略は良好な性能を示しており、監視下での段階的導入を推奨")
        else:
    ...
    def _identify_critical_issues(self, test_results: List[EnsembleTestResult]) -> List[str]:
        ...
        if len(failed_results) > len(test_results) * 0.25:  # 25%以上失敗
            critical_issues.append(f"高い失敗率: {len(failed_results)}/{len(test_results)}件のテストが失敗")
