...
class MLIntegrationTester:
    ...
    def run_full_integration_test(
        ...
        try:
            ...
            if run_backtest:
                logger.info(
                    "テスト5: 高度バックテストエンジン", section="integration_test"
                )
            ...
            successful_tests = sum(
                1
                for result in test_report["test_results"].values()
            ...
            test_report["execution_time"] = (
                test_report["end_time"] - test_report["start_time"]
            ).total_seconds()
    ...
    def _test_data_quality_enhancer(self, symbol: str) -> Dict:
        ...
        try:
            ...
            # 結果検証
            original_outliers = (
                (test_data["Close"] - test_data["Close"].median()).abs()
                > test_data["Close"].std() * 3
            ).sum()
            cleaned_outliers = (
                (cleaned_data["Close"] - cleaned_data["Close"].median()).abs()
                > cleaned_data["Close"].std() * 3
            ).sum()
            ...
            result = {
                ...
                "missing_values_filled": test_data.isnull().sum().sum() > 0
                and cleaned_data.isnull().sum().sum() == 0,
            }

            logger.info(
                "データ品質向上テスト完了", section="data_quality_test", **result
            )
            ...
            logger.error(
                f"データ品質向上テストエラー: {e}", section="data_quality_test"
            )
    ...
    def _test_feature_engineering(self, symbol: str) -> Dict:
        ...
        try:
            ...
            result = {
                "success": len(found_features)
                >= len(expected_features) * 0.7,  # 70%以上存在
    ...
    def _test_advanced_backtest(self, symbol: str) -> Dict:
        ...
        try:
            ...
            result = {
                ...
                "realistic_costs_applied": performance.total_commission > 0
                or performance.total_slippage > 0,
            }
    ...
    def generate_test_report(self, test_results: Dict) -> str:
        ...
        report_lines.extend(["", "=" * 60, "テスト完了", "=" * 60])
...
def run_ml_integration_demo():
    ...
    try:
        ...
        test_results = tester.run_full_integration_test(
            symbols=["7203", "8306"], run_ml_training=True, run_backtest=True
        )
