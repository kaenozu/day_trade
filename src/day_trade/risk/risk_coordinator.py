...
from ..monitoring.metrics.decorators import (
    measure_risk_analysis_performance,
    track_errors,
)
from ..monitoring.metrics.prometheus_metrics import get_health_metrics, get_risk_metrics
...
from .generative_ai_engine import (
...
class RiskAnalysisCoordinator:
    ...
    def _create_fraud_detection_request(
        self, request_id: str, transaction_data: Dict[str, Any]
    ...
    async def _run_ai_analysis(
        ...
        try:
            result = await self.generative_ai_engine.analyze_risk_comprehensive(
                request, use_gpt4=True, use_claude=True, use_ensemble=True
            )
    ...
    async def _integrate_analysis_results(
        ...
        if risk_category in ["high", "critical"]:
            self.risk_metrics.risk_alerts_total.labels(
                alert_level=risk_category, component="risk_coordinator"
            ).inc()
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        total = self.performance_stats["total_assessments"]
        success_rate = self.performance_stats["successful_assessments"] / max(1, total)
    ...
    async def batch_risk_assessment(
        self, transactions: List[Dict[str, Any]], concurrent_limit: int = 10
