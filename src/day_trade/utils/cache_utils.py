            ...
            for i, arg in enumerate(args):
                try:
                    normalized_items.append(
                        _normalize_arguments(arg, max_depth, current_depth + 1, seen_objects)
                    )
                except CacheError:
                    # CacheErrorは再発生
                    raise
        ...
        # datetime/date/time オブジェクト（タイムゾーン情報保持）
            ...
            try:
                ...
                return {
                    ...
                    "__timezone__": (
                        str(getattr(obj, "tzinfo", None)) if hasattr(obj, "tzinfo") else None
                    ),
                }
            except Exception:
    ...
    def get_peak_operations_per_second(self, window_seconds: float = 10.0) -> float:
        ...
        def calculate_peak_ops():
            ...
            for i in range(len(self._operation_times)):
                ...
                ops_in_window = sum(
                    1 for op_time in self._operation_times if window_start <= op_time <= window_end
                )
