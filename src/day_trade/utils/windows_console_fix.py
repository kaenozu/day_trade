...
def setup_windows_console():
    ...
    try:
        ...
        logger.info("Windows コンソール環境を初期化しました",
                   platform="win32",
                   encoding="utf-8",
                   ansi_enabled=True)

    except Exception as e:
        logger.warning("Windows コンソール初期化で警告",
                      error=str(e),
                      fallback="デフォルト設定で継続")
...
def create_safe_live_context():
    ...
    def safe_live_context(*args, **kwargs):
        ...
        try:
            ...
            # Liveインスタンスカウンターを減少
            safe_live_context._active_count = max(0, getattr(safe_live_context, '_active_count', 1) - 1)
