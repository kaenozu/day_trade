...
try:
    from .next_gen_hft_engine import (
        NextGenHFTEngine, NextGenHFTConfig, NextGenExecutionOrder,
        HFTExecutionMode, HFTMarketData, MarketRegime,
        create_next_gen_hft_engine
    )
    from .ai_market_predictor import (
        AIMarketPredictor, PredictionModel, MarketPrediction,
...
class IntegratedHFTTestSuite:
    ...
    async def _execute_test_orders(self, hft_engine: NextGenHFTEngine, ai_predictor: Optional[AIMarketPredictor], scenario: TestScenario, result: HFTTestResult) -> bool:
        """テストオーダー実行"""
        orders_submitted = 0
        latency_samples = []
        ...
        try:
            for i in range(scenario.order_count):
                ...
                order = NextGenExecutionOrder(
                    ...
                    symbol=symbol,
                    side="BUY" if (not ai_recommendation or ai_recommendation.recommended_action == "BUY") else "SELL" if ai_recommendation.recommended_action == "SELL" else ("BUY" if i % 2 == 0 else "SELL"),
            ...
            try:
                for symbol in symbols:
                    ...
                    market_data = HFTMarketData(
                        ...
                        market_regime=MarketRegime.NORMAL if sequence % 100 < 80 else MarketRegime.VOLATILE
                    )
    ...
    async def _collect_performance_stats(self, hft_engine: NextGenHFTEngine, ai_predictor: Optional[AIMarketPredictor], result: HFTTestResult):
        """パフォーマンス統計収集"""
        try:
            # HFTエンジン統計
    ...
    async def _generate_comprehensive_report(self, total_test_time: float) -> Dict[str, Any]:
        ...
        report = {
            ...
            "performance_overview": {
                ...
                "overall_success_rate_percent": round(total_successful / max(total_orders, 1) * 100, 1),
    ...
    def _analyze_optimization_effects(self) -> Dict[str, Any]:
        ...
        no_batch_tests = [r for r in self.test_results if "ultra_low_latency" in r.test_name and r.success]
        ...
        if batch_enabled_tests and no_batch_tests:
            ...
            if no_batch_avg_latency > 0:
                latency_impact = round((batch_avg_latency - no_batch_avg_latency) / no_batch_avg_latency * 100, 1)

            batch_effect = {
                "batch_optimization_enabled": {
                    "avg_latency_us": batch_avg_latency,
                    "avg_throughput": statistics.mean([r.throughput_ops for r in batch_enabled_tests])
                },
        ...
        ai_effect = {}
        if ai_enabled_tests:
            ai_accuracies = [r.ai_prediction_accuracy for r in ai_enabled_tests if r.ai_prediction_accuracy > 0]
        ...
        return {
            ...
            "cache_performance": {
                "avg_hit_rate_percent": cache_avg
            }
        }
    ...
    def print_summary_report(self, report: Dict[str, Any]):
        ...
        print(f"テスト実行結果: {summary['passed_tests']}/{summary['total_tests']} passed ({summary['pass_rate_percent']}%)")
