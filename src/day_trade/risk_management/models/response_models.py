...
from .unified_models import (
    ...
    ProcessingStats, AssetType, RiskLevel, AnalysisStatus
)
...
class BatchAnalysisResponse(BaseResponse):
    ...
    def get_failed_results(self) -> List[ErrorResponse]:
        """失敗結果の取得"""
        return [result for result in self.individual_results
...
def create_portfolio_risk_response(
    ...
    return PortfolioRiskResponse(
        ...
        confidence_score=analysis_result.risk_metrics.confidence_score if analysis_result.risk_metrics else 0.0,
    ...
    def to_dict(response: BaseResponse) -> Dict[str, Any]:
        ...
        for field_name in response.__dataclass_fields__:
            ...
            if isinstance(value, Enum):
                ...
                result[field_name] = [
                    ResponseConverter.to_dict(item) if hasattr(item, '__dataclass_fields__') else item
                    for item in value
                ]
            else:
