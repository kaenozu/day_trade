...
from opentelemetry.metrics import Counter, Histogram, Gauge, ObservableCounter, ObservableGauge

from .telemetry_config import get_meter, get_logger
...
class MetricsCollector:
    ...
    def increment_counter(self, name: str, value: float = 1.0, attributes: Optional[Dict[str, Any]] = None) -> None:
        """カウンター増加"""
    ...
    def record_hft_latency(self, operation: str, latency_microseconds: float) -> None:
        ...
        self.record_histogram('trade_latency', latency_microseconds, {
            'operation': operation
        })

    def get_hft_latency_percentiles(self, operation: str, percentiles: List[float] = None) -> Dict[str, float]:
        """HFT用レイテンシパーセンタイル計算"""
        if percentiles is None:
            percentiles = [50.0, 95.0, 99.0, 99.9]

        with self._metric_locks[f'hft_latency_{operation}']:
            latencies = [item['latency_us'] for item in self._hft_metrics[f'hft_latency_{operation}']]
        ...
        return {
            f'p{p}': np.percentile(latencies_array, p)
    ...
    def register_slo(self, slo_config: SLOConfig) -> None:
        ...
        self.logger.info("SLO registered", slo_name=slo_config.name, target=slo_config.target_percentage)
    ...
    def calculate_sli(self, slo_name: str, time_window_seconds: Optional[int] = None) -> float:
        ...
        with self._metric_locks[f'sli_{slo_name}']:
            recent_events = [
                event for event in self._sli_data[slo_name]
    ...
    def measure_operation(self, operation_name: str, attributes: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            self.increment_counter('errors_total', attributes={**attributes, 'error_type': type(e).__name__})
...
def get_metrics_collector(service_name: str = "day-trade") -> MetricsCollector:
    ...
    if _global_metrics_collector is None:
        ...
        # 標準SLO設定
        _global_metrics_collector.register_slo(SLOConfig(
