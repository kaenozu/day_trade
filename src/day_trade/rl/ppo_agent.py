...
class PPOAgent:
    ...
    def __init__(self, env: MultiAssetTradingEnvironment, config: Optional[PPOConfig] = None):
        self.env = env
        ...
        # ネットワーク初期化
        if PYTORCH_AVAILABLE:
            self.actor_critic = ActorCriticNetwork(self.state_dim, self.action_dim, self.config).to(
                self.device
            )
