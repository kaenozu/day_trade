...
class StyleAnalyzer:
    ...
    def analyze_portfolio_style(self,
                              portfolio_data: pd.DataFrame,
                              benchmark_data: pd.DataFrame = None,
                              holdings_data: pd.DataFrame = None) -> StyleAnalysisResult:
        """ポートフォリオスタイル分析"""
        ...
        try:
            ...
            logger.info(f"スタイル分析完了: {detected_style.value} (信頼度: {style_confidence:.2f})")
            return result
    ...
    def _calculate_factor_exposures(self,
        ...
        if benchmark_data is not None and len(benchmark_data) == len(portfolio_data):
            ...
            if len(returns) == len(benchmark_returns) and len(returns) > 1:
                ...
                exposures['beta'] = covariance / benchmark_variance if benchmark_variance > 0 else 1.0
            else:
                exposures['beta'] = 1.0
        else:
            exposures['beta'] = 1.0
    ...
    def _classify_investment_style(self,
                                 factor_exposures: Dict[str, float],
                                 returns: pd.Series) -> Dict[InvestmentStyle, float]:
        """投資スタイル分類"""
        ...
        for style, definition in self.style_definitions.items():
            ...
            if typical_vol_range[0] <= portfolio_vol <= typical_vol_range[1]:
                ...
                vol_distance = min(abs(portfolio_vol - typical_vol_range[0]),
    ...
    def _calculate_style_consistency(self,
                                   returns: pd.Series,
                                   detected_style: InvestmentStyle) -> float:
        """スタイル整合性計算"""
    ...
    def train_style_classifier(self,
                             ...
                             style_labels: Dict[str, InvestmentStyle]) -> Dict[str, Any]:
        """スタイル分類器訓練"""
        ...
        try:
            ...
            # 特徴量重要度
            feature_names = ['volatility', 'momentum_12m', 'momentum_3m', 'beta',
                           'trend_strength', 'mean_reversion', 'skewness', 'kurtosis']
            feature_importance = dict(zip(feature_names[:X.shape[1]],
    ...
    def recommend_style_adaptation(self,
