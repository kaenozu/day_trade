...
class RiskParityOptimizer:
    ...
    def optimize(self,
                ...
                custom_covariance: pd.DataFrame = None) -> OptimizationResult:
        """リスクパリティ最適化実行"""
    ...
    def _equal_risk_contribution_optimization(self,
                                            cov_matrix: np.ndarray,
                                            asset_names: List[str]) -> OptimizationResult:
        """等リスク寄与度最適化"""
        ...
        return self._create_result(result.x, cov_matrix, asset_names, result.success, result.nit, result.fun)

    def _risk_budgeting_optimization(self,
                                   cov_matrix: np.ndarray,
                                   asset_names: List[str]) -> OptimizationResult:
        """リスクバジェット最適化"""
        ...
        # 目標リスク寄与度配列
        target_contrib = np.array([risk_budgets.get(name, 1.0/len(asset_names))
        ...
        optimization_result = self._create_result(result.x, cov_matrix, asset_names, result.success, result.nit, result.fun)
    ...
    def _hierarchical_risk_parity(self,
        ...
        try:
            ...
            for cluster_id in range(1, self.config.n_clusters + 1):
                ...
                if len(cluster_assets) == 1:
                    ...
                    cluster_result = self._equal_risk_contribution_optimization(
                        cluster_cov,
                        [asset_names[i] for i in cluster_assets]
                    )

                    # クラスタ重みで調整
                    for i, asset_idx in enumerate(cluster_assets):
                        asset_name = asset_names[asset_idx]
                        final_weights[asset_idx] = cluster_result.weights[asset_name] * cluster_weights
            ...
            return self._equal_risk_contribution_optimization(
                self._estimate_covariance_matrix(returns_data),
                asset_names
            )
    ...
    def _create_result(self,
                      ...
                      success: bool = True,
                      iterations: int = 0,
                      objective_value: float = 0.0) -> OptimizationResult:
        """最適化結果作成"""
    ...
    def _validate_and_adjust_result(self,
        ...
        if portfolio_vol > 1e-8:
            ...
            result.risk_contributions = {name: contrib for name, contrib in zip(asset_names, risk_contrib)}
    ...
    def get_optimization_summary(self) -> Dict[str, Any]:
        ...
        # 成功率
        successful_optimizations = sum(1 for r in self.optimization_history if r.optimization_success)
