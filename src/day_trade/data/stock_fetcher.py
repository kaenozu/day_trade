...
class StockFetcher:
            ...
            try:
                ...
                if self._is_retryable_error(e):
                    if attempt < self.retry_count - 1:
                        logger.warning(f"リトライ {attempt + 1}/{self.retry_count}: {e}")
    ...
    def _validate_date_range(self, start_date: Union[str, datetime], end_date: Union[str, datetime]) -> None:
        """日付範囲の妥当性をチェック"""
    ...
    def _validate_period_interval(self, period: str, interval: str) -> None:
        ...
        if interval.endswith("m") and period not in ["1d", "5d"]:
            raise InvalidSymbolError(f"分足データは1d または 5d 期間のみサポートされています")
