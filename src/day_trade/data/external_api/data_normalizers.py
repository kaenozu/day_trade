...
class DataNormalizer:
    ...
    def __init__(self):
        self.sanitize_stats = {"sanitized_errors": 0}
    ...
    def _sanitize_error_message(self, error_message: str, error_type: str) -> str:
        """エラーメッセージの機密情報サニタイゼーション"""
        # 安全なエラーメッセージのマッピング
        safe_messages = {
            "ParserError": "CSVデータの解析でエラーが発生しました",
            "ValueError": "データの値が不正です",
            "default": "データ処理でエラーが発生しました",
        }

        self.sanitize_stats["sanitized_errors"] += 1
        return safe_messages.get(error_type, safe_messages["default"])

    def get_stats(self) -> dict:
        """統計取得"""
        return self.sanitize_stats.copy()