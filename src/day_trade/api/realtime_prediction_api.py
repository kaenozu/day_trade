...
from ..ml import DeepLearningConfig, DeepLearningModelManager
...
class RealtimePredictionAPI:
                ...
                return jsonify(
                    {
                        ...
                        "successful_predictions": len(
                            [r for r in batch_results if r.get("error") is None]
                        ),
                    }
                )
    ...
    def _execute_prediction(
        ...
        try:
            ...
            confidence_intervals = self._calculate_confidence_intervals(
                prediction_result, request.confidence_level
            )
        ...
        for symbol, future in futures:
            try:
                ...
                if result:
                    ...
                    results.append({"symbol": symbol, "error": "予測に失敗しました"})
            except Exception as e:
    ...
    def _prepare_models(self, sample_data: pd.DataFrame):
        ...
        try:
            ...
            # Transformerモデル追加
            from ..ml import LSTMModel, TransformerModel
    ...
    def _calculate_confidence_intervals(
        self, prediction_result, confidence_level: float
    ) -> List[Dict[str, float]]:
        """信頼区間計算"""
        try:
            ...
            if prediction_result.uncertainty:
                ...
                for i, (pred, std) in enumerate(
                    zip(
                        prediction_result.predictions,
                        prediction_result.uncertainty.upper_bound
                        - prediction_result.uncertainty.lower_bound,
                    )
                ):
