...
class ErrorAnalyzer:
    ...
    def _classify_error(self, error_message: str, error_type: str) -> tuple:
        ...
        for category_name, category_info in self.error_patterns.items():
            for pattern in category_info["patterns"]:
                if (
                    pattern["keyword"].lower() in error_message.lower()
                    or pattern["keyword"] in error_type
                ):
    ...
    def _determine_severity(
        self, exception: Exception, category: ErrorCategory, context: Dict[str, Any]
    ) -> ErrorSeverity:
        """重要度判定"""
        # 致命的エラー
        ...
        **recovery_kwargs,
        ...
        if hasattr(error_context, "error_subtype"):
            localized_message = self.message_manager.get_message(
                error_context.category, error_context.error_subtype, self.language
            )
