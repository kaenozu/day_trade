...
class AlertManager:
    ...
    def get_alerts(self, symbol: Optional[str] = None) -> List[AlertCondition]:
        """アラート条件を取得"""
        if symbol:
            return [condition for condition in self.alert_conditions.values() 
    ...
    def check_all_alerts(self):
        """全アラートのチェック"""
        active_conditions = [condition for condition in self.alert_conditions.values() 
            ...
            for condition in conditions:
                if self._should_check_condition(condition):
                    trigger = self._evaluate_condition(
                        condition, current_price, volume, change_percent, historical_data
                    )
    ...
    def _evaluate_condition(
        ...
        try:
            ...
            if condition.alert_type == AlertType.PRICE_ABOVE:
                ...
                if self._compare_values(current_price, condition.condition_value, condition.comparison_operator):
                ...
                if condition.custom_function:
                    result = condition.custom_function(
                        condition.symbol, current_price, volume, change_percent, 
                        historical_data, condition.custom_parameters
                    )
                    if result:
                        current_value = "Custom"
                        message = f"カスタム条件が満たされました: {condition.description}"
    ...
    def _compare_values(self, current: Union[Decimal, float], target: Union[Decimal, float, str], operator: str) -> bool:
        """値の比較"""
        try:
    ...
    def _validate_condition(self, condition: AlertCondition) -> bool:
        ...
        if condition.alert_type == AlertType.CUSTOM_CONDITION and not condition.custom_function:
            return False
    ...
    def get_alert_history(
        ...
        filtered_history = [
            trigger for trigger in self.alert_history
            if trigger.trigger_time >= cutoff_time
        ]
        
        if symbol:
            filtered_history = [
                trigger for trigger in filtered_history
        ...
        try:
            ...
            for alert_id, condition in self.alert_conditions.items():
                config_data[alert_id] = {
                    ...
                    'expiry_date': condition.expiry_date.isoformat() if condition.expiry_date else None,
