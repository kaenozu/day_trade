...
class TradeManager:
    ...
    def __init__(self, commission_rate: Decimal = Decimal('0.001'), tax_rate: Decimal = Decimal('0.2')):
        """
        初期化
    ...
    def add_trade(
        ...
        try:
            ...
            logger.info(f"取引追加: {trade_id} - {symbol} {trade_type.value} {quantity}株 @{price}円")
            return trade_id
    ...
    def _update_position(self, trade: Trade):
        ...
        if trade.trade_type == TradeType.BUY:
            if symbol in self.positions:
                ...
                total_cost = position.total_cost + (trade.price * Decimal(trade.quantity)) + trade.commission
            ...
            if symbol in self.positions:
                ...
                if position.quantity >= trade.quantity:
                    ...
                    # 手数料を按分
                    buy_commission_per_share = position.total_cost / Decimal(position.quantity) - position.average_price
                    buy_commission = buy_commission_per_share * Decimal(trade.quantity)
                    
                    pnl_before_tax = (sell_price - buy_price) * Decimal(trade.quantity) - buy_commission - trade.commission
                    ...
                    if remaining_quantity > 0:
                        # 按分してコストを調整
                        remaining_ratio = Decimal(remaining_quantity) / Decimal(position.quantity)
                        remaining_cost = position.total_cost * remaining_ratio
                        position.quantity = remaining_quantity
                        position.total_cost = remaining_cost
                        position.average_price = remaining_cost / Decimal(remaining_quantity)
                    else:
                        # ポジション完全クローズ
                        del self.positions[symbol]
                
                else:
                    logger.warning(f"売却数量が保有数量を超過: {symbol} 保有:{position.quantity} 売却:{trade.quantity}")
            else:
                logger.warning(f"ポジションが存在しない銘柄の売却: {symbol}")
    
    def _get_earliest_buy_date(self, symbol: str) -> datetime:
        """最も古い買い取引の日付を取得"""
        buy_trades = [t for t in self.trades if t.symbol == symbol and t.trade_type == TradeType.BUY]
        if buy_trades:
    ...
    def get_portfolio_summary(self) -> Dict:
        ...
        return {
            ...
            'win_rate': f"{(len([pnl for pnl in self.realized_pnl if pnl.pnl > 0]) / max(len(self.realized_pnl), 1) * 100):.1f}%" if self.realized_pnl else "0.0%"
        }
    ...
    def export_to_csv(self, filepath: str, data_type: str = 'trades'):
        ...
        try:
            if data_type == 'trades':
                ...
                fieldnames = ['id', 'symbol', 'trade_type', 'quantity', 'price', 'timestamp', 
                             'commission', 'status', 'notes']
            
            elif data_type == 'positions':
                data = [pos.to_dict() for pos in self.positions.values()]
                fieldnames = ['symbol', 'quantity', 'average_price', 'total_cost', 'current_price',
                ...
                fieldnames = ['symbol', 'quantity', 'buy_price', 'sell_price', 'buy_commission',
                             'sell_commission', 'pnl', 'pnl_percent', 'buy_date', 'sell_date']
    ...
    def load_from_json(self, filepath: str):
        ...
        try:
            ...
            for pnl_data in data.get('realized_pnl', []):
                self.realized_pnl.append(RealizedPnL(
    ...
    def calculate_tax_implications(self, year: int) -> Dict:
        ...
        try:
            ...
            total_gain = sum(pnl.pnl for pnl in year_pnl if pnl.pnl > 0) if year_pnl else Decimal('0')
            total_loss = sum(abs(pnl.pnl) for pnl in year_pnl if pnl.pnl < 0) if year_pnl else Decimal('0')
...
if __name__ == "__main__":
    ...
    if position:
        ...
        print(f"含み損益: {position.unrealized_pnl}円 ({position.unrealized_pnl_percent}%)")
