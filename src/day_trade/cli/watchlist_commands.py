...
from ..utils.formatters import create_error_panel, create_success_panel, get_change_color, format_currency, format_percentage, format_volume
...
def watchlist_add(codes, group: str, memo: str):
    ...
    # 結果表示
    if success_count > 0:
        console.print(create_success_panel(f"{success_count}件の銘柄をウォッチリスト（{group}）に追加しました。"))
    
    if failed_codes:
        console.print(create_error_panel(f"追加に失敗した銘柄: {', '.join(failed_codes)}"))
...
def watchlist_remove(codes, group: str):
    ...
    # 結果表示
    if success_count > 0:
        console.print(create_success_panel(f"{success_count}件の銘柄をウォッチリスト（{group}）から削除しました。"))
    
    if failed_codes:
        console.print(create_error_panel(f"削除に失敗した銘柄: {', '.join(failed_codes)}"))
    ...
    if prices:
        ...
        for code, item in data.items():
            ...
            table.add_row(
                ...
                item.get('memo', '')[:20] + '...' if len(item.get('memo', '')) > 20 else item.get('memo', '')
            )
...
def watchlist_memo(code: str, group: str, text: str):
    ...
    if manager.update_memo(code, group, text):
        ...
        console.print(create_error_panel(f"銘柄 {code} が見つからないか、更新に失敗しました。"))
...
def watchlist_move(code: str, to_group: str, from_group: str):
    ...
    if manager.move_to_group(code, from_group, to_group):
        console.print(create_success_panel(f"銘柄 {code} を {from_group} から {to_group} に移動しました。"))
    else:
        console.print(create_error_panel(f"移動に失敗しました。銘柄が見つからないか、移動先に既に存在する可能性があります。"))
