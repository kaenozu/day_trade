...
def test_api_endpoint_configuration():
    ...
    endpoints = [
        ...
        APIEndpoint("secondary", "https://api-backup.example.com", priority=2, timeout=20.0),
        APIEndpoint("tertiary", "https://api-fallback.example.com", priority=3, timeout=15.0),
    ]
...
def test_resilient_client_status():
    ...
    for endpoint_status in status["endpoints"]:
        ...
        print(f"  エンドポイント {endpoint_status['name']}: {endpoint_status['circuit_state']}")
...
def test_fallback_strategies():
    ...
    assert list(default_historical.columns) == ['Open', 'High', 'Low', 'Close', 'Volume']
...
def test_validation_enhancements():
    ...
    assert abs(price_data["change_percent"] - 1.3513513513513513) < 0.001  # (20/1480)*100
