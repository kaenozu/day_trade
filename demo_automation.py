...
def demo_config_manager():
    ...
    try:
        ...
        for symbol in symbols:
            print(f"  {symbol.code}: {symbol.name} ({symbol.group}) [優先度: {symbol.priority}]")
    ...
    try:
        ...
        if report.generated_signals:
            ...
            for signal in report.generated_signals:
                print(f"  {signal['symbol']}: {signal['type']} - {signal['reason']} (信頼度: {signal['confidence']:.2f})")
...
def demo_command_line():
    ...
    examples = [
        {
            "command": "python daytrade.py",
            "description": "デフォルト設定で全自動実行"
        },
        {
...
def demo_report_generation():
    ...
    try:
        ...
        print(f"\n[レポート] レポート形式:")
        print(f"  [OK] JSON形式 - 機械読み取り用の詳細データ")  
        print(f"  [OK] CSV形式 - Excel等での分析用")
        print(f"  [OK] HTML形式 - ブラウザで見やすい表示")
        
        print(f"\n[銘柄] レポート内容:")
        print(f"  • 実行サマリー（成功/失敗銘柄数、実行時間）")
        print(f"  • 生成シグナル一覧（BUY/SELL推奨）")
        print(f"  • アラート発生履歴")
        print(f"  • ポートフォリオ状況")
        print(f"  • エラー詳細")
