...
class TestUpdateFromDict:
    ...
    def test_update_with_exclusions(self, sample_user):
        ...
        update_data = {"username": "should_not_update", "age": 30}
    ...
    def test_datetime_string_conversion(self, sample_user):
        """datetime文字列の自動変換テスト"""
        update_data = {"created_at": "2023-01-01T12:00:00+00:00"}
...
class TestDatabaseIntegration:
    ...
    def test_database_persistence(self, test_db_session, sample_user):
        ...
        # データベースから取得
        retrieved_user = (
            test_db_session.query(DummyUser).filter_by(username="test_user").first()
        )
    ...
    def test_timezone_persistence(self, test_db_session, sample_user):
        ...
        # データベースから取得
        retrieved_user = (
            test_db_session.query(DummyUser).filter_by(id=sample_user.id).first()
        )
        ...
        if retrieved_user.created_at.tzinfo is None:
            ...
            assert (
                retrieved_user.created_at.replace(tzinfo=timezone.utc)
                == original_created_at
            )
            assert (
                retrieved_user.updated_at.replace(tzinfo=timezone.utc)
                == original_updated_at
            )
        else:
    ...
    def test_decimal_precision(self, test_db_session):
        ...
        product = DummyProduct(
            name="Test Product", price=Decimal("99.99"), description="Test description"
        )
        ...
        # データベースから取得
        retrieved_product = (
            test_db_session.query(DummyProduct).filter_by(id=product.id).first()
        )
