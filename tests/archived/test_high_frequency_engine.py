...
async def run_performance_benchmark():
    ...
    config = OptimizationConfig(
        enable_gpu=True, enable_caching=True, cache_ttl_seconds=300
    )
    ...
    try:
        ...
        print(
            f"     - メモリ割り当て平均時間: {memory_test_results['avg_allocation_us']:.2f}μs"
        )
        print(
            f"     - メモリ解放平均時間: {memory_test_results['avg_deallocation_us']:.2f}μs"
        )
        ...
        print(
            f"     - キュー投入平均時間: {queue_test_results['avg_enqueue_us']:.2f}μs"
        )
        print(
            f"     - キュー取得平均時間: {queue_test_results['avg_dequeue_us']:.2f}μs"
        )
        ...
        print(
            f"     - 決定処理平均時間: {decision_test_results['avg_decision_us']:.2f}μs"
        )
        ...
        print("  📡 市場データフィード:")
        print(
            f"     - 受信ティック数: {detailed_stats['market_data']['ticks_received']:,}"
        )
        print(
            f"     - 平均遅延: {detailed_stats['market_data']['avg_latency_us']:.1f}μs"
        )
        print(f"     - 最大遅延: {detailed_stats['market_data']['max_latency_us']}μs")

        print("  🧠 決定エンジン:")
        print(
            f"     - 決定回数: {detailed_stats['decision_engine']['decisions_made']:,}"
        )
        print(
            f"     - 平均決定時間: {detailed_stats['decision_engine']['avg_decision_time_us']:.1f}μs"
        )
        ...
        print(
            f"     - 割り当て済みブロック: {detailed_stats['memory_pool']['allocated_blocks']}"
        )
        ...
        print(
            f"  {throughput_emoji} スループット評価: {throughput_grade} ({throughput:.0f} 注文/秒)"
        )
        ...
        print(
            f"  {stability_emoji} 安定性評価: {stability_grade} ({error_rate:.3f}% エラー率)"
        )
...
async def test_order_queue_performance(engine) -> dict:
    ...
    return {
        ...
        "avg_dequeue_us": (
            sum(dequeue_times) / len(dequeue_times) if dequeue_times else 0
        ),
