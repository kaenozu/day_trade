    ...
    try:
        ...
        for i, (symbol, data) in enumerate(successful_data.items(), 1):
            ...
            if i % 5 == 0:
                ...
                print(
                    f"  進捗: {i}/{len(successful_data)} ({elapsed:.2f}s, {rate:.1f}銘柄/s)"
                )
        ...
        for i, (symbol, result) in enumerate(results.items()):
            if i >= 10:
                break
            print(
                f"{symbol:>6} | {result['advice']:>5} | {result['confidence']:>4.0f}% | {result['reason']:>15}"
            )
...
def main():
    ...
    if result and stress_result:
        ...
        print(
            f"85銘柄処理推定時間: {min_estimated_time:.1f}秒 〜 {max_estimated_time:.1f}秒"
        )
