...
async def test_performance_alert_system() -> Dict[str, Any]:
    ...
    try:
        ...
        alert_summary = {
            ...
            "critical_alerts": len(
                [a for a in performance_alerts if a["severity"] == "critical"]
            ),
            "warning_alerts": len(
                [a for a in performance_alerts if a["severity"] == "warning"]
            ),
        }
...
async def test_investment_opportunity_alert_system() -> Dict[str, Any]:
    ...
    try:
        ...
        opportunity_stats = {
            ...
            "high_profit_opportunities": len(
                [
                    opp
                    for opp in investment_opportunities
                    if opp["profit_potential"] > 10.0
                ]
            ),
            "avg_confidence_score": sum(
                opp["confidence_score"] for opp in investment_opportunities
            )
            / len(investment_opportunities),
            "avg_profit_potential": sum(
                opp["profit_potential"] for opp in investment_opportunities
            )
            / len(investment_opportunities),
        }
        ...
        return {"phase": "Phase 4: 投資機会アラート", "success": False, "error": str(e)}
...
async def main():
    ...
    for i, result in enumerate(test_results, 1):
        ...
        if success:
            ...
            if "healthy_components" in result:
                print(
                    f"   ヘルス監視: {result['healthy_components']}/{result['total_components']} 正常"
                )
                print(f"   アラート生成数: {result['alerts_generated']}")

            if "performance_metrics" in result:
                print(
                    f"   パフォーマンススコア: {result.get('overall_performance_score', 0):.3f}"
                )
                print(
                    f"   アラート数: {result.get('alert_summary', {}).get('total_alerts', 0)}"
                )
            ...
            if "investment_opportunities" in result:
                print(
                    f"   投資機会数: {result.get('opportunity_stats', {}).get('total_opportunities', 0)}"
                )
                avg_confidence = result.get("opportunity_stats", {}).get(
    ...
    if health_result and health_result["success"]:
        health_rate = (
            health_result["healthy_components"] / health_result["total_components"]
        )
        health_target_met = health_rate >= 0.95  # 95%以上
        print(
            f"  1. システム稼働率95%以上: {health_rate:.1%} {'OK' if health_target_met else 'NG'}"
        )
    else:
    ...
    if perf_result and perf_result["success"]:
        ...
        print(
            f"  2. パフォーマンス監視70%スコア: {perf_score:.3f} {'OK' if perf_target_met else 'NG'}"
        )
    else:
    ...
    if quality_result and quality_result["success"]:
        ...
        print(
            f"  3. データ品質85%スコア: {quality_score:.3f} {'OK' if quality_target_met else 'NG'}"
        )
    else:
    ...
    # 投資機会検出 (Phase 4)
    opp_result = next(
        (r for r in test_results if "investment_opportunities" in r), None
    )
    if opp_result and opp_result["success"]:
        opp_count = opp_result.get("opportunity_stats", {}).get(
            "total_opportunities", 0
        )
        avg_confidence = opp_result.get("opportunity_stats", {}).get(
            "avg_confidence_score", 0
        )
        opp_target_met = opp_count >= 1 and avg_confidence >= 0.6
        print(
            f"  4. 投資機会検出機能: {opp_count}件検出, 信頼度{avg_confidence:.3f} {'OK' if opp_target_met else 'NG'}"
        )
    else:
