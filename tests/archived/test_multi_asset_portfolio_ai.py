...
class MultiAssetPortfolioAITester:
    ...
    async def _generate_test_data(self) -> Dict[str, Any]:
        ...
        for asset, info in assets.items():
            ...
            daily_returns = np.random.normal(
                base_return * dt, volatility * np.sqrt(dt), n_periods
            )
        ...
        all_prices = pd.DataFrame(
            {asset: data["Close"] for asset, data in price_data.items()}
        )
        ...
        try:
            ...
            for asset in test_assets:
                ...
                results = indicator_engine.calculate_indicators(
                    data=asset_data, symbols=[asset], timeframe="1D"
                )
                ...
                for expected in expected_indicators:
                    ...
                    logger.warning(
                        f"期待される指標が見つかりません: {expected_indicators}"
                    )
            ...
            assert (
                performance_summary["total_indicators"] > 50
            ), "指標数が不足しています"
    ...
    async def _test_risk_parity_optimization(self, test_data: Dict[str, Any]):
        ...
        try:
            ...
            assert (
                len(result.weights) == test_data["n_assets"]
            ), "重み数が資産数と一致しません"
            assert (
                abs(sum(result.weights.values()) - 1.0) < 1e-6
            ), "重み合計が1.0になりません"
            assert (
                result.portfolio_volatility > 0
            ), "ポートフォリオボラティリティが無効です"

            # 制約チェック
            for weight in result.weights.values():
                assert (
                    config.min_weight <= weight <= config.max_weight
                ), f"重み制約違反: {weight}"

            # リスク寄与度チェック
            risk_contributions = list(result.risk_contributions.values())
            assert len(risk_contributions) == len(
                result.weights
            ), "リスク寄与度数が一致しません"
            assert (
                abs(sum(risk_contributions) - 1.0) < 1e-3
            ), "リスク寄与度合計が1.0になりません"
            ...
            assert (
                "diversification_metrics" in analysis
            ), "分散化メトリクスが含まれていません"
    ...
    async def _test_style_analyzer(self, test_data: Dict[str, Any]):
        ...
        try:
            ...
            assert isinstance(
                result.detected_style, InvestmentStyle
            ), "スタイル検出結果が無効です"
            assert 0 <= result.style_confidence <= 1, "信頼度が範囲外です"
            assert isinstance(
                result.risk_profile, RiskProfile
            ), "リスクプロファイルが無効です"
            assert result.volatility > 0, "ボラティリティが無効です"

            # ファクター露出チェック
            assert (
                "volatility" in result.factor_exposures
            ), "ボラティリティ露出が含まれていません"
            assert "beta" in result.factor_exposures, "ベータ露出が含まれていません"

            # 機械学習予測テスト（訓練データなしでフォールバック）
            ml_style, ml_confidence = analyzer.predict_style_ml(
                portfolio_prices.to_frame("portfolio")
            )
    ...
    async def _test_ai_portfolio_manager(self, test_data: Dict[str, Any]):
        ...
        try:
            ...
            historical_data = {
                asset: data["Close"] for asset, data in test_data["price_data"].items()
            }

            # ポートフォリオ初期化
            success = await manager.initialize_portfolio(
                asset_universe, historical_data
            )
            ...
            assert (
                optimization_result.convergence_achieved
            ), "最適化が収束しませんでした"
            assert len(optimization_result.allocations) == len(
                asset_universe
            ), "配分数が資産数と一致しません"

            total_weight = sum(
                alloc.weight for alloc in optimization_result.allocations
            )
            assert (
                abs(total_weight - 1.0) < 1e-6
            ), f"重み合計が1.0になりません: {total_weight}"
    ...
    async def _test_system_integration(self, test_data: Dict[str, Any]):
        ...
        try:
            ...
            indicator_results = indicator_engine.calculate_indicators(
                data=test_data["price_data"][sample_asset], symbols=[sample_asset]
            )
            ...
            assert (
                0.05 < style_volatility < 1.0
            ), f"スタイル分析ボラティリティ異常: {style_volatility}"
            assert (
                0.05 < risk_parity_volatility < 1.0
            ), f"リスクパリティボラティリティ異常: {risk_parity_volatility}"
    ...
    async def _test_system_performance(self, test_data: Dict[str, Any]):
        ...
        try:
            ...
            indicator_results = indicator_engine.calculate_indicators(
                data=sample_data, symbols=["test_asset"]
            )
            ...
            logger.info(
                f"[{'PASS' if all_passed else 'WARN'}] システムパフォーマンステスト完了"
            )
    ...
    async def _print_test_summary(self):
        ...
        logger.info("\n" + "=" * 80)
        logger.info(
            "[SUMMARY] マルチアセット・ポートフォリオAIシステム テスト結果サマリー"
        )
        logger.info("=" * 80)
        ...
        logger.info(
            f"成功率: {(passed_tests/(total_tests-skipped_tests))*100:.1f}%"
        ...
        logger.info(
            "- 100+指標分析: トレンド・モメンタム・ボラティリティ・出来高・パターン"
        )
