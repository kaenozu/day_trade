    ...
    data = pd.DataFrame(
        {
            ...
            "Sector": np.random.choice(
                ["Finance", "IT", "Manufacturing", "Retail"], rows
            ),  # category candidate
        }
    )
...
def test_vectorization():
    ...
    print(
        f"  Average time per operation: {vectorization_time*1000/len(technical_operations):.2f}ms"
    )
    ...
    return {
        ...
        "avg_time_per_operation_ms": round(
            vectorization_time * 1000 / len(technical_operations), 2
        ),
...
def generate_performance_report(
    ...
    print(
        "  | Data Size | Before(MB) | After(MB) | Saved(MB) | Reduction(%) | Time(ms) |"
    )
    print("  |---|---|---|---|---|---|")
    for result in dtype_results:
        print(
            f"  | {result['size']:,} rows | {result['memory_before_mb']:.3f} | {result['memory_after_mb']:.3f} | {result['memory_saved_mb']:.3f} | {result['reduction_percent']:.1f} | {result['optimization_time_ms']:.2f} |"
        )
    ...
    print(
        f"  - Average time per operation: {vectorization_result['avg_time_per_operation_ms']:.2f}ms"
    )
    ...
    print(
        f"  - Memory reduction by dtype optimization: Average {avg_memory_reduction:.1f}%"
    )
    print(
        f"  - Speed improvement by vectorization: {vectorization_result['total_operations']} operations in {vectorization_result['vectorization_time_ms']:.2f}ms"
    )
    print(
        f"  - Large data processing by chunking: {chunk_result['data_size']:,} rows in {chunk_result['chunk_processing_time_ms']:.2f}ms"
    )
