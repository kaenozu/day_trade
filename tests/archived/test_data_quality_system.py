...
def test_basic_data_quality():
    ...
    # 3. 異常値チェック
    negative_prices = dirty_data[
        (dirty_data["high"] < 0) | (dirty_data["low"] < 0) | (dirty_data["close"] < 0)
    ]
    ...
    print(
        f"  欠損値: {dirty_data.isnull().sum().sum()} → {cleaned_data.isnull().sum().sum()}"
    )
...
async def test_advanced_data_system():
    ...
    try:
        ...
        dq_system = ComprehensiveDataQualitySystem(
            base_path=Path("test_data_quality"), enable_monitoring=True
        )
        ...
        quality_result = await dq_system.assess_data_quality(
            data=dirty_data, dataset_name="test_financial_data"
        )
...
def test_data_validation_rules():
    ...
    # 金融データ特有のバリデーションルール
    validation_rules = {
        "price_positive": lambda df: (df[["open", "high", "low", "close"]] >= 0)
        .all()
        .all(),
        ...
        "no_missing_critical": lambda df: df[["symbol", "date", "close"]]
        .isnull()
        .sum()
        .sum()
        == 0,
    ...
    for rule_name, rule_func in validation_rules.items():
        try:
            ...
            print(
                f"  正常データ - {rule_name}: {'[合格] PASS' if is_valid else '[不合格] FAIL'}"
            )

            is_valid_dirty = rule_func(dirty_data)
            print(
                f"  問題データ - {rule_name}: {'[合格] PASS' if is_valid_dirty else '[不合格] FAIL'}"
            )
...
def main():
    ...
    print(
        f"  バリデーションルール: {'[合格] PASS' if validation_result else '[不合格] FAIL'}"
    )
