    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        pipeline_config = PipelineConfig(
            feature_store_config=FeatureStoreConfig(
                base_path=temp_dir, enable_compression=True, max_cache_size_mb=100
            ),
            optimization_config=OptimizationConfig(
                level=OptimizationLevel.ADAPTIVE, performance_monitoring=True
            ),
            batch_size=10,
        )
        ...
        batch_results = pipeline.batch_generate_features(test_data, feature_config)
...
def test_deduplication_system():
    ...
    # 1回目のリクエスト
    is_dup_1, task_key_1 = dedup_manager.is_duplicate_request(
        symbol, data, feature_config
    )
    print(
        f"  First request - duplicate: {is_dup_1}, task_key: {task_key_1[:8] if task_key_1 else None}"
    )

    # タスク登録
    registered_key = dedup_manager.register_computation_task(
        symbol, data, feature_config
    )
    print(f"  Registered task key: {registered_key[:8]}")

    # 2回目のリクエスト（重複のはず）
    is_dup_2, task_key_2 = dedup_manager.is_duplicate_request(
        symbol, data, feature_config
    )
    print(
        f"  Second request - duplicate: {is_dup_2}, task_key: {task_key_2[:8] if task_key_2 else None}"
    )

    # 3回目の登録（重複検出のはず）
    registered_key_2 = dedup_manager.register_computation_task(
        symbol, data, feature_config
    )
...
def test_end_to_end_feature_efficiency():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with FeaturePipeline(pipeline_config) as pipeline:
            ...
            first_results = pipeline.batch_generate_features(test_data, feature_config)
            ...
            second_results = pipeline.batch_generate_features(test_data, feature_config)
            ...
            third_results = pipeline.batch_generate_features(mixed_data, feature_config)
            ...
            print(
                f"  Deduplication rate: {dedup_stats['deduplication_rate_percent']:.1f}%"
            )
            print(
                f"  Time saved by deduplication: {dedup_stats['time_saved_by_dedup_seconds']:.2f}s"
            )
...
def generate_performance_report(
    store_results, pipeline_results, dedup_results, e2e_results
):
    """パフォーマンスレポート生成"""
    print("\n" + "=" * 80)
    ...
    print(
        f"  - Cache hit rate: {store_results['cache_stats']['cache_hit_rate_percent']:.1f}%"
    )
    ...
    print(
        f"  - Processing rate: {pipeline_results['symbols_processed']/pipeline_results['first_batch_time']:.1f} symbols/s"
    )
    ...
    total_time_with_optimization = (
        store_results["first_run_time"]
        + store_results["second_run_time"] * 2
        + pipeline_results["first_batch_time"]
        + pipeline_results["second_batch_time"]
        + sum(
            [
                e2e_results["first_execution_time"],
                e2e_results["second_execution_time"],
                e2e_results["third_execution_time"],
            ]
        )
    )
