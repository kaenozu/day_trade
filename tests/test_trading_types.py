...
from day_trade.core.types.trading_types import (
    TradingConstraints, RiskParameters, OrderConstraints,
    TypeSafeTradeManager, TradeValidationError,
    OrderLimitExceededError, RiskLimitExceededError,
    MarketHours, TradingSession, OrderBookEntry,
    PriceLevel, VolumeAnalysis, LiquidityMetrics
)
...
class TestTradingConstraints:
    ...
    def test_constraints_creation(self):
        ...
        assert constraints.max_position_size.amount == Decimal("1000000")
        assert constraints.max_daily_loss.amount == Decimal("50000")
        assert constraints.max_single_trade_size.amount == Decimal("100000")
        assert constraints.min_trade_size.amount == Decimal("1000")
        assert "7203" in constraints.allowed_symbols
        assert constraints.max_orders_per_day == 100
    ...
    def test_validate_order_valid(self):
        ...
        constraints.validate_order(symbol, quantity, price)
    ...
    def test_validate_order_too_small(self):
        ...
        with pytest.raises(TradeValidationError, match="too small"):
            symbol = Symbol("7203")
            quantity = Quantity(5)  # 5 * 1500 = 7500 < 10000
            price = Price(Decimal("1500"), "JPY")
            constraints.validate_order(symbol, quantity, price)
    ...
    def test_validate_order_too_large(self):
        ...
        with pytest.raises(TradeValidationError, match="too large"):
            symbol = Symbol("7203")
            quantity = Quantity(100)  # 100 * 1500 = 150000 > 100000
            price = Price(Decimal("1500"), "JPY")
            constraints.validate_order(symbol, quantity, price)
...
class TestRiskParameters:
    ...
    def test_risk_parameters_creation(self):
        ...
        assert risk_params.max_portfolio_value.amount == Decimal("5000000")
        assert risk_params.max_daily_var.amount == Decimal("100000")
        assert risk_params.position_size_limit_percent == Decimal("10")
        assert risk_params.stop_loss_percent == Decimal("5")
        assert risk_params.take_profit_percent == Decimal("15")
        assert risk_params.max_correlation == Decimal("0.7")

    def test_calculate_position_size_limit(self):
        """ポジションサイズ制限計算テスト"""
        risk_params = RiskParameters(
            max_portfolio_value=Money(Decimal("1000000"), "JPY"),
            max_daily_var=Money(Decimal("50000"), "JPY"),
...
class TestTypeafeTradeManager:
    ...
    def trade_manager(self):
        ...
        return TypeSafeTradeManager(constraints, risk_params, order_constraints)
    ...
    def test_place_market_order_valid(self, trade_manager):
        ...
        with patch.object(trade_manager, '_get_current_market_price') as mock_price:
            ...
            order = trade_manager.place_market_order(symbol, quantity, is_buy)
    ...
    def test_place_limit_order_valid(self, trade_manager):
        ...
        with patch.object(trade_manager, '_get_current_market_price') as mock_market_price:
            mock_market_price.return_value = Price(Decimal("1510"), "JPY")

            order = trade_manager.place_limit_order(symbol, quantity, price, is_buy)
    ...
    def test_cancel_order(self, trade_manager):
        ...
        with patch.object(trade_manager, '_get_current_market_price') as mock_market_price:
            ...
            order = trade_manager.place_limit_order(symbol, quantity, price, True)
            order_id = order["order_id"]

            cancelled = trade_manager.cancel_order(order_id)
            assert cancelled is True

            # 存在しない注文のキャンセル
            non_existent = trade_manager.cancel_order("non_existent_id")
