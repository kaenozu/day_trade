...
from src.day_trade.core.alerts import (
    AlertManager, AlertCondition, AlertTrigger, NotificationHandler,
    AlertType, AlertPriority, NotificationMethod,
    create_price_alert, create_change_alert
)
...
class TestNotificationHandler:
    ...
    def test_file_log_notification(self, tmp_path):
        ...
        try:
            ...
            # ログファイルが作成されることを確認
            log_files = [f for f in os.listdir('.') if f.startswith('alerts_') and f.endswith('.log')]
    ...
    def test_custom_handler(self):
        ...
        self.handler.send_notification(self.sample_trigger, [NotificationMethod.WEBHOOK])
...
class TestAlertManager:
    ...
    def test_add_alert(self):
        ...
        assert self.alert_manager.alert_conditions["test_condition"] == self.sample_condition
    ...
    def test_get_alerts(self):
        ...
        condition1 = AlertCondition(
            alert_id="alert1", symbol="7203", alert_type=AlertType.PRICE_ABOVE, condition_value=3000
        )
        condition2 = AlertCondition(
            alert_id="alert2", symbol="9984", alert_type=AlertType.PRICE_BELOW, condition_value=1500
        )
        condition3 = AlertCondition(
            alert_id="alert3", symbol="7203", alert_type=AlertType.CHANGE_PERCENT_UP, condition_value=5
        )
    ...
    def test_is_expired(self):
        ...
        condition_no_expiry = AlertCondition(
            alert_id="no_expiry", symbol="7203", alert_type=AlertType.PRICE_ABOVE, condition_value=3000
        )
        assert self.alert_manager._is_expired(condition_no_expiry) == False
        
        # 未来の期限
        future_expiry = AlertCondition(
            alert_id="future_expiry", symbol="7203", alert_type=AlertType.PRICE_ABOVE, condition_value=3000,
        ...
        past_expiry = AlertCondition(
            alert_id="past_expiry", symbol="7203", alert_type=AlertType.PRICE_ABOVE, condition_value=3000,
            expiry_date=datetime.now() - timedelta(hours=1)
        )
    ...
    def test_validate_condition(self):
        ...
        valid_condition = AlertCondition(
            alert_id="valid", symbol="7203", alert_type=AlertType.PRICE_ABOVE, condition_value=3000
        )
        assert self.alert_manager._validate_condition(valid_condition) == True
        
        # IDなし
        no_id = AlertCondition(
            alert_id="", symbol="7203", alert_type=AlertType.PRICE_ABOVE, condition_value=3000
        )
        assert self.alert_manager._validate_condition(no_id) == False
        
        # 銘柄なし
        no_symbol = AlertCondition(
            alert_id="test", symbol="", alert_type=AlertType.PRICE_ABOVE, condition_value=3000
        )
        assert self.alert_manager._validate_condition(no_symbol) == False
        
        # カスタム条件で関数なし
        custom_no_func = AlertCondition(
            alert_id="custom", symbol="7203", alert_type=AlertType.CUSTOM_CONDITION, condition_value="test"
        )
    ...
    def test_should_check_condition(self):
        ...
        active_condition = AlertCondition(
            alert_id="active", symbol="7203", alert_type=AlertType.PRICE_ABOVE, 
            condition_value=3000, is_active=True
        )
        assert self.alert_manager._should_check_condition(active_condition) == True
        
        # 非アクティブな条件
        inactive_condition = AlertCondition(
            alert_id="inactive", symbol="7203", alert_type=AlertType.PRICE_ABOVE, 
            condition_value=3000, is_active=False
        )
        assert self.alert_manager._should_check_condition(inactive_condition) == False
        
        # クールダウン中
        cooldown_condition = AlertCondition(
            alert_id="cooldown", symbol="7203", alert_type=AlertType.PRICE_ABOVE, 
            condition_value=3000, cooldown_minutes=60
        )
        ...
        # クールダウン時間経過後はチェック可能
        self.alert_manager.last_trigger_times["cooldown"] = datetime.now() - timedelta(hours=2)
        assert self.alert_manager._should_check_condition(cooldown_condition) == True
    ...
    def test_evaluate_price_condition(self):
        ...
        price_above_condition = AlertCondition(
            alert_id="price_above", symbol="7203", alert_type=AlertType.PRICE_ABOVE,
            condition_value=Decimal('3000'), comparison_operator=">"
        )
    ...
    def test_evaluate_change_percent_condition(self):
        ...
        change_up_condition = AlertCondition(
            alert_id="change_up", symbol="7203", alert_type=AlertType.CHANGE_PERCENT_UP,
            condition_value=5.0
        )
    ...
    def test_evaluate_custom_condition(self):
        ...
        custom_condition = AlertCondition(
            alert_id="custom", symbol="7203", alert_type=AlertType.CUSTOM_CONDITION,
            condition_value="custom", custom_function=custom_func,
    ...
    def test_alert_history(self):
        ...
        trigger1 = AlertTrigger(
            alert_id="hist1", symbol="7203", trigger_time=datetime.now(),
            alert_type=AlertType.PRICE_ABOVE, current_value=3100, condition_value=3000,
            message="テスト1", priority=AlertPriority.MEDIUM
        )
        
        trigger2 = AlertTrigger(
            alert_id="hist2", symbol="9984", trigger_time=datetime.now() - timedelta(hours=2),
            alert_type=AlertType.PRICE_BELOW, current_value=1400, condition_value=1500,
            message="テスト2", priority=AlertPriority.HIGH
        )
