...
class TestInteractiveMode:
    ...
    def setup_method(self):
        ...
        with patch('src.day_trade.cli.interactive.WatchlistManager', return_value=self.mock_watchlist_manager):
            with patch('src.day_trade.cli.interactive.TradeManager', return_value=self.mock_trade_manager):
                with patch('src.day_trade.cli.interactive.StockFetcher', return_value=self.mock_stock_fetcher):
                    with patch('src.day_trade.cli.interactive.TradingSignalGenerator', return_value=self.mock_signal_generator):
    ...
    def test_load_initial_data_with_existing_watchlist(self):
        ...
        mock_alerts = [
            {'id': 1, 'stock_code': '7203', 'alert_type': 'price_above', 'threshold': 3000}
        ]
        ...
        assert self.interactive.current_data['portfolio_summary']['total_positions'] == 2
    ...
    def test_load_initial_data_empty_watchlist(self):
        ...
        assert self.mock_watchlist_manager.add_stock.call_count == 3  # 3つのサンプル銘柄
    ...
    def test_create_watchlist_summary_with_data(self):
        ...
        mock_watchlist = [
            {'stock_code': '7203', 'stock_name': 'トヨタ自動車', 'group_name': '自動車'},
            {'stock_code': '8306', 'stock_name': '三菱UFJ銀行', 'group_name': '銀行'},
            {'stock_code': '9984', 'stock_name': 'ソフトバンクグループ', 'group_name': 'テック'},
        ]
    ...
    def test_background_update_thread_functionality(self):
        ...
        thread = threading.Thread(target=self.interactive._background_update, daemon=True)
        thread.start()
...
class TestInteractiveModeIntegration:
    ...
    def test_full_initialization_flow(self, mock_signal_gen, mock_fetcher, mock_trade_mgr, mock_watchlist_mgr):
        """完全初期化フローテスト"""
        # モックインスタンスを設定
    ...
    def test_error_handling_in_data_update(self):
        """データ更新時のエラーハンドリングテスト"""
        with patch('src.day_trade.cli.interactive.WatchlistManager') as mock_watchlist_mgr:
            with patch('src.day_trade.cli.interactive.TradeManager') as mock_trade_mgr:
                ...
                mock_watchlist_instance.get_watchlist.side_effect = Exception("Test error")
                mock_watchlist_instance.get_alerts.return_value = []
                mock_trade_instance.get_portfolio_summary.return_value = {}
