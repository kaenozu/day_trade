...
from day_trade.models import init_db, reset_db, db_manager
from day_trade.data.stock_master import StockMasterManager, stock_master
from day_trade.models.stock import Stock
...
class TestStockMasterManager:
    ...
    def test_search_stocks_by_name(self, setup_test_db, sample_stocks):
        ...
        results = test_manager._search_with_db_manager(
            lambda session: session.query(Stock).filter(
                Stock.name.ilike("%ソ%")
            ).all()
        )
