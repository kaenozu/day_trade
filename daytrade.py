...
def setup_logging(log_level: str = "INFO"):
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(f'daytrade_{datetime.now().strftime("%Y%m%d")}.log', encoding='utf-8')
        ]
    )
...
def print_summary(report):
    ...
    if report.generated_signals:
        ...
        for i, signal in enumerate(report.generated_signals[:5], 1):  # 上位5件
            print(f"  {i}. {signal['symbol']} - {signal['type']} ({signal['reason']}) [信頼度: {signal['confidence']:.2f}]")
    ...
    if report.portfolio_summary and 'metrics' in report.portfolio_summary:
        ...
        print(f"  総損益: {metrics.get('total_pnl', 'N/A')}円 ({metrics.get('total_pnl_percent', 'N/A')}%)")
...
使用例:
  ...
  python daytrade.py --log-level DEBUG       # デバッグモード
    ...
    parser.add_argument(
        '--symbols',
        type=str,
        help='対象銘柄をカンマ区切りで指定 (例: 7203,8306,9984)'
    )
    ...
    parser.add_argument(
        '--version',
        action='version',
        version='DayTrade Auto Engine v1.0.0'
    )
