...
from src.day_trade.utils.formatters import (
    # 基本フォーマッタ
    format_currency, format_percentage, format_volume, format_large_number,
    create_stock_info_table, create_watchlist_table,
    
    # 高度なASCII表示機能
    create_ascii_chart, create_sparkline, create_heatmap,
    create_progress_bar_panel, create_comparison_table,
...
def demo_progress_and_status():
    ...
    console.print(Panel(
        Text.assemble(*[Text.assemble(status, "\n") for status in status_content[:-1]], status_content[-1]),
        title="🚦 システムステータス",
        border_style="blue"
    ))
...
def demo_trend_indicators():
    ...
    console.print(Panel(
        Text.assemble(*[Text.assemble(trend, "\n") for trend in trend_content[:-1]], trend_content[-1]),
        title="📈 市場トレンド指標",
        border_style="cyan"
    ))
...
def interactive_demo():
    ...
    def create_dashboard():
        ...
        chart = create_ascii_chart(price_data, width=60, height=8, title="📊 リアルタイム価格チャート")
        ...
        layout.split_column(
            Layout(Panel(metric_cards, title="📊 市場メトリクス", border_style="blue"), size=8),
            Layout(Panel(chart, title="📈 チャート表示", border_style="green"), size=12),
            Layout(Panel("\n".join(sparkline_content), title="⚡ 主要指標", border_style="yellow"), size=6)
        )
