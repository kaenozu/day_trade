...
def create_sample_market_data() -> MarketData:
    ...
    volume_data = pd.DataFrame({"Volume": [100000, 120000, 90000, 150000, 110000]})
...
def create_sample_portfolio_data() -> PortfolioData:
    ...
    return PortfolioData(
        ...
        allocation={"7203.T": 0.30, "8306.T": 0.15, "9984.T": 0.35, "6758.T": 0.20},
        performance_metrics={
            "total_return": 0.08,
            "sharpe_ratio": 0.75,
            "max_drawdown": -0.12,
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for plugin_info in plugins:
                print(
                    f"- {plugin_info.name} v{plugin_info.version} ({plugin_info.plugin_type.value})"
                )
...
def test_builtin_plugins():
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            context = {"daily_trades": 25, "news_data": market_data.news_data}

            # 全プラグインで分析実行
            results = manager.analyze_with_all_plugins(
                market_data=market_data, portfolio_data=portfolio_data, context=context
            )
...
"""
            ...
            initial_count = len(
                [p for p in initial_plugins if p.name == "Custom Test Plugin"]
            )
            ...
            reloaded_count = len(
                [p for p in reloaded_plugins if p.name == "Custom Test Plugin"]
            )
...
def main():
    ...
    print("-" * 60)
    print(
        f"総合結果: {success_count}/{total_count} ({success_count/total_count*100:.1f}%)"
    )
