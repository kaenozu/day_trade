...
class ChartWidget:
    ...
    def update_chart(self, cpu: float, memory: float):
        ...
        if len(self.data_history["cpu"]) > 1:
            # CPU使用率グラフ
            self.ax1.plot(
                self.data_history["timestamps"],
                self.data_history["cpu"],
                "b-",
                linewidth=2,
            )
            self.ax1.set_title("CPU使用率 (%)")
...
class ControlPanelWidget:
    ...
    def apply_settings(self):
        ...
        try:
            ...
            self.app.log_viewer.add_log(
                "INFO",
                f"設定を適用しました: 監視間隔={interval}秒, ログレベル={log_level}",
            )
                ...
                if cpu_percent > 80 or memory_mb > 15000:
                    ...
                    if cpu_percent > 90:
                        self.log_viewer.add_log(
                            "WARNING", f"CPU使用率が高すぎます: {cpu_percent:.1f}%"
                        )
